...
protocol RSLoadingViewEffect {
  func setup(_ main: RSLoadingView)
  func prepareForResize(_ main: RSLoadingView)
  func update(at time: TimeInterval)
}

open class RSLoadingView: UIView, SCNSceneRendererDelegate {
  
  public enum Effect: String {
    case spinAlone, twins
  }
  
  @IBInspectable open var speedFactor: CGFloat = 1.0
  @IBInspectable open var mainColor: UIColor = UIColor.white
  @IBInspectable open var colorVariation: CGFloat = 0.0
  @IBInspectable open var sizeFactor: CGFloat = 1.0
  @IBInspectable open var spreadingFactor: CGFloat = 1.0
  @IBInspectable open var lifeSpanFactor: CGFloat = 1.0
  @IBInspectable open var variantKey: String = ""
  ...
  open var shouldDimBackground = true
  open var dimBackgroundColor = UIColor.black.withAlphaComponent(0.6)
  open var isBlocking = true
  open var shouldTapToDismiss = false
  open var sizeInContainer: CGSize = CGSize(width: 180, height: 180)
  ...
  func loadParticleSystem(_ name: String) -> SCNParticleSystem? {
  ...
  func loadParticleImage(_ name: String) -> UIImage? {
  ...
  static open func hideFromKeyWindow() {
    hide(from: UIApplication.shared.keyWindow!)
  }
  
  static open func hide(from view: UIView) {
    let containerViews = view.subviews.filter { (view) -> Bool in
      return view is RSLoadingContainerView
    }
    containerViews.forEach { (view) in
